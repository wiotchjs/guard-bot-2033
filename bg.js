...
**Yetkilinin yetkileri alınıp karantinaya atıldı!**`)
    ...
    client4.on("channelDelete", async channel => {
        let entry = await channel.guild.fetchAuditLogs({ type: 'CHANNEL_DELETE' }).then(audit => audit.entries.first());
        if (!entry || !entry.executor || Date.now() - entry.createdTimestamp > 10000) return;
        if (matthe2.bots.includes(entry.executor.id)) return;
        if (matthe2.owners.includes(entry.executor.id)) return;
  ...
  **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
        channel2.send({ embed: jahky })
        return client4.users.cache.get(ayarlar2.sahip).send(`**Sunucuda kanal silindi! Silen kişinin bilgileri :** \n**Kullanıcı Adı :** \`\`${entry.executor.tag}\`\` **Kullanıcı İdsi :** \`\`${entry.executor.id}\`\`\n**Kanal Adı :**\`\`${channel.name}\`\` **Kanal İdsi :** \`\`${channel.id}\`\``)
    });
    ...
    client3.on("channelCreate", async channel => {
        ...
        channel.guild.members.ban(entry.executor.id, { reason: `İzinsiz Kanal Oluşturma!` });
        channel.delete({ reason: "Kanal Koruma Sistemi!" });
        let channel2 = client3.channels.cache.get(ayarlar2.log1)
        if (!channel2) return console.log('Kanal Koruma Logu Yok!');
        const jahky = new Discord2.MessageEmbed()
            .setTimestamp()
            .setColor(ayarlar2.color)
  ...
  **Emoji eski haline getirildi ve yetkili jaile atıldı!**`)
    ...
    **Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
    ...
    client.login("token 1").catch(err => {
    ...
    client1.login("token 2").catch(err => {
    ...
    client2.login("token 3").catch(err => {
        console.error("Guard III Giriş Yapamadı!")
        console.error("Token Girilmemiş!")
    });
  
      client3.on("ready", () => {
        client3.user.setPresence({ activity: { name: ayarlar2.durum }, status: ayarlar2.status });
        console.log(`${client3.user.username} Olarak Giriş Yapıldı Guard IV aktif`);
    })

    client3.login("token 4").catch(err => {
        console.error("Guard IV Giriş Yapamadı!")
        console.error("Token Girilmemiş!")
    });
  
      client4.on("ready", () => {
        client4.user.setPresence({ activity: { name: ayarlar2.durum }, status: ayarlar2.status });
        console.log(`${client4.user.username} Olarak Giriş Yapıldı Guard V aktif`);
    })

    client4.login("token 5").catch(err => {
        console.error("Guard V Giriş Yapamadı!")
        console.error("Token Girilmemiş!")
    });
